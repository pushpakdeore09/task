...
public class ResourceController {
    ...
    public ResponseEntity<Map<String, String>> updateResource(@RequestBody ResourceRequest resourceRequest){
        Map<String, String> response = new HashMap<>();
        try{
            String status = resourceService.updateResource(resourceRequest);
            response.put("message", status);
            return new ResponseEntity<>(response, HttpStatus.OK);
        } catch (Exception e){
            response.put("error", e.getMessage());
